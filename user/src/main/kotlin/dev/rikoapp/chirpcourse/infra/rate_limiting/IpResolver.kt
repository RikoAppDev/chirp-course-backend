package dev.rikoapp.chirpcourse.infra.rate_limiting

import dev.rikoapp.chirpcourse.infra.config.NginxConfig
...
class IpResolver(
    ...
    private fun validateAndNormalizeIp(ip: String, headerName: String, proxyIp: String): String? {
        ...
        if (trimmedIp.isBlank() || INVALID_IPS.contains(trimmedIp)) {
            logger.warn("Invalid IP in $headerName header from proxy $proxyIp: '$ip'")
            return null
        }
        ...
        return try {
            val inetAddr = when {
                ...
                trimmedIp.matches(Regex("\\d+\\.\\d+\\.\\d+\\.\\d+")) -> Inet4Address.getByName(trimmedIp)
                    ...
                    logger.warn("Invalid IP format in $headerName header from proxy $proxyIp: '$ip'")
                    return null
                }
            }

            if (isPrivateIp(inetAddr.hostName)) {
                logger.warn("Private IP in $headerName header from proxy $proxyIp: '$ip'")
            }
